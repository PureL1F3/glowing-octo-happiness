...
class VidblitDB
{
    ...
    function request_status($userid, $requestid)
    {
        ...
        $sql = "SELECT e.id, e.type, e.title, e.length, e.error, e.location from request r left join extract e on r.extractid=e.id where r.id=$sql_requestid and r.userid=$sql_userid";
        $result = $this->_con->query($sql);
        if(!$result)
        {
            return $this->mysql_error();
        ...
        if($object)
        {
            ...
            $value = array('id' => $object->id, 'type' => $object->type, 'title' => $object->title, 'length' => $object->length, 'error' => $object->error, 'available' => !is_null($object->location));
        }
        return $this->result(true, $value);
    }
